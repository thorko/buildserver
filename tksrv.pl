...
sub print_help {
	print "Usage: tkctl.pl [-c config file] -r <command>";
	return 0;
}
...
$config = defined($config) && $config ne "" ? $config : "/etc/tkctl/tksrv.conf";
...
=head1 DESCRIPTION
 ...
 tksrv.pl is a tool to serve applications from a build server
